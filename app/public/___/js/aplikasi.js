$(document).ready(function () {
  ...
  if (url == "m_siswa") {
    pagination("datatabel", base_url + "adm/m_siswa/getSiswa", []);
  } else if (url == "m_guru") {
  ...
  } else if (url == "h_ujian_peserta") {
    if (url2 == "cetak") {
      pagination(
        "datatabel",
        base_url + "adm/h_ujian_peserta/cetak/" + url3,
        []
      );
    } else {
      pagination("datatabel", base_url + "adm/h_ujian_peserta/data", []);
    }
  } else if (url == "ikut_ujian") {
    if (url2 == "token") {
      timer();
    }
  } else if (url == "m_akses_soal") {
    if (url2 == "cetak") {
      pagination("datatabel", base_url + "adm/h_ujian_peserta/cetak/" + url3, [
        {
          targets: [0, 2],
          orderable: false,
        },
      ...
      pagination("datatabel", base_url + "adm/m_akses_soal/data", []);
    }
  }
});

function maskInput(selector){
	$("#"+ selector ).inputmask({
...
function checkKarakter(str) {
	// Mengecek panjang string
	var panjangKarakter = str.length;
  
	// Menampilkan pesan jika panjang karakter kurang dari 16
	if (panjangKarakter < 16) {
	  alert("Jumlah karakter minimal 16!");
	  return false;
	}
  
	// Menampilkan pesan jika panjang karakter lebih dari 16
	if (panjangKarakter > 16) {
	  alert("Jumlah karakter maksimal 16!");
	  return false;
	}
  
	// Mengembalikan true jika panjang karakter sesuai
	return true;
  }
...
function login(e) {
  ...
  $.ajax({
    ...
    success: function (r) {
      if (r.log.status == 0) {
        $("#konfirmasi").html(
          "<div class='alert alert-danger'>" + r.log.keterangan + "</div>"
        );
      } else {
        $("#konfirmasi").html(
          "<div class='alert alert-success'>" + r.log.keterangan + "</div>"
        );
...
function m_siswa_e(id) {
  ...
  $.ajax({
    ...
    success: function (data) {
      console.log('tgl lahir', data);
	  if(id > 0){
		var a = data.tgl_lahir;
		$("#id").val(data.id);
		$("#nama").val(data.nama);
		$("#nim").val(data.nim);
...
function m_siswa_h(id) {
  if (confirm("Anda yakin..? Jika terhapus data ujian akan ikut terhapus")) {
    $.ajax({
      type: "GET",
...
function m_siswa_u(id) {
  if (confirm("Anda yakin..? Username dan Password otomatis adalah NIK ..!")) {
    $.ajax({
      type: "GET",
...
function m_akses_s() {
  ...
  $.ajax({
    type: "POST",
    url: base_url + "adm/m_akses_soal/simpan",
    data: JSON.stringify(form),
    dataType: "json",
    contentType: "application/json; charset=utf-8",
  }).done(function (response) {
    if (response.status == "ok") {
      window.location.assign(base_url + "adm/m_akses_soal");
    } else {
...
function m_akses_h(id, id_guru, id_mapel) {
  //url = base_url + "adm/m_akses_soal/hapus?id=" + id + "&id_guru=" + id_guru + "&id_mapel=" + id_mapel
  //alert(url);
  if (confirm("Anda yakin..?")) {
    $.ajax({
      type: "GET",
      ...
      success: function (response) {
        if (response.status == "ok") {
          ...
          // console.log('gagal');
          alert("Data tidak bisa dihapus karena terintegrasi pada soal");
        }
      },
    });
  }
...
function __ambil_jumlah_soal(id_mapel) {
  let id = id_mapel ? id_mapel : 0;
  $.ajax({
    type: "GET",
    url: base_url + "adm/m_ujian/jumlah_soal/" + id,
    success: function (response) {
